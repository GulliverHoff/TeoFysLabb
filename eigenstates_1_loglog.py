...
for N in N_values:
    ...
    for i in range(N):
        ...
        x_tab.append(x / a)
        psi_tab.append(psi)
    Error_N.append(abs(psi))

Error_expected = []
C=4.5e-10
for N in N_values:
    Error_expected.append(C/N**2)

print("")

plt.close()
plt.loglog()
plt.plot(N_values, Error_N, linewidth=2)
plt.plot(N_values, Error_expected, linewidth=1, color="green")
plt.xlabel("N", fontsize=15)
plt.ylabel("$|\psi{(x=a)}|$", fontsize=15)
# plt.savefig('psi.pdf')
plt.show()
